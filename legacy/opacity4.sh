...
# This version uses hyprctl activewindow and setprop, with full paths and a delay.

LOG_FILE="/tmp/hypr_opacity_script_v3.log"
# Replace with the actual paths found using 'which jq' and 'which hyprctl'
...
CURRENT_MULTIPLIER=$(${HYPRCTL_PATH} getprop address:"$ACTIVE_WINDOW_ADDRESS" opacity -j | ${JQ_PATH} -r '.float')
...
if [[ -z "$CURRENT_MULTIPLIER" || "$CURRENT_MULTIPLIER" == "null" ]]; then
  ...
  log_message "Failed to get current window opacity multiplier. Assuming 1.0. Output: $(${HYPRCTL_PATH} getprop address:"$ACTIVE_WINDOW_ADDRESS" opacity -j)"
else
...
${HYPRCTL_PATH} setprop address:"$ACTIVE_WINDOW_ADDRESS" opacity "$NEW_MULTIPLIER"
if [ $? -eq 0 ]; then
